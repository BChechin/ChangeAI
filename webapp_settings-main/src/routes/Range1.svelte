...
<div class="range">
	<div class="range__wrapper" tabindex="0" on:keydown="{onKeyPress}" bind:this="{element}" role="slider" aria-valuemin="{min}" aria-valuemax="{max}" aria-valuenow="{value}" id="{id}" on:mousedown="{onTrackEvent}" on:touchstart="{onTrackEvent}">
		<div class="range__track" bind:this="{container}">
			...
			<div class="range__thumb" class:range__thumb--holding="{holding}" bind:this="{thumb}" on:touchstart="{onDragStart}" on:mousedown="{onDragStart}" on:mouseover="{() => (thumbHover = true)}" on:mouseout="{() => (thumbHover = false)}">
...
<style>
    ...
    outline: none; */
		@apply min-w-full relative py-2 px-[4px] box-border outline-none;
	}
	...
	.range__track--highlighted {
		...
		@apply w-0 h-[6px] absolute bg-tg-button;
	}

	.range__thumb {
		background-color: var(--thumb-bgcolor, rgba(255, 255, 255, 0.379));
		transition: box-shadow 100ms;
		box-shadow: var(--thumb-boxshadow, 0 1px 1px 0 rgba(0, 0, 0, 0.14), 0 0px 2px 1px rgba(0, 0, 0, 0.2));
		@apply flex items-center justify-center absolute w-2 h-[22px] cursor-pointer rounded-sm -mt-[8px] select-none z-[1000];
	}

	.range__thumb--holding {
		/* box-shadow: 0 1px 1px 0 rgba(0, 0, 0, 0.14), 0 1px 2px 1px rgba(0, 0, 0, 0.2), 0 0 0 6px var(--thumb-holding-outline, rgba(113, 120, 250, 0.558)); */
	}
